...
| **Critic**     | Reviews the findings — if too thin, sends the pipeline back to the Retriever for another pass (up to 2 retries) before proceeding |
...
Create a `.env` file in the project root (`research-platform/.env`):
...
The Critic/Retriever loop is capped (default: 2 retries) to guarantee the pipeline always terminates, even if the findings never reach a "sufficient" verdict.
